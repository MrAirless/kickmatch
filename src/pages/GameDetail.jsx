...
export default function GameDetail() {
  ...
  async function handleBooking(formData) {
    ...
    if (game.anbieter_email) {
      supabase.from('notifications').insert([{
        user_email: game.anbieter_email, game_id: String(id),
        type: 'neue_anfrage', title: 'Neue Anfrage',
        message: `${formData.verein} · ${formatDate(game.datum)}`,
      }]).then(() => {}).catch(() => {})
      fetch('/api/send-push', {
        method: 'POST',
  ...
  async function handleAnnehmen(anfrage) {
    ...
    if (anfrage.bucher_email) {
      supabase.from('notifications').insert([{
        user_email: anfrage.bucher_email, game_id: String(id),
        type: 'angenommen', title: '🎉 Anfrage angenommen',
        message: `${game.verein} · ${formatDate(game.datum)}`,
      }]).then(() => {}).catch(() => {})
      fetch('/api/send-push', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    anfragen.filter((a) => a.id !== anfrage.id && a.bucher_email).forEach((a) => {
      supabase.from('notifications').insert([{
        user_email: a.bucher_email, game_id: String(id),
        type: 'abgelehnt', title: 'Anfrage abgelehnt',
        message: `${game.verein} · ${formatDate(game.datum)}`,
      }]).then(() => {}).catch(() => {})
    })
  ...
  async function handleOnlineStellen() {
    ...
    if (aktuelleBuchungen) {
      aktuelleBuchungen.filter((b) => b.status === "angenommen" && b.bucher_email).forEach((b) => {
        supabase.from('notifications').insert([{
          user_email: b.bucher_email, game_id: String(id),
