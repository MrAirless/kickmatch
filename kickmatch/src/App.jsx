...
const INITIAL_GAMES = [
  {
    id: 1, type: "angebot", jugend: "E-Jugend (U10)",
    datum: "2026-05-10", uhrzeit: "10:00",
    platz: "Sportpark Nord", adresse: "Stuttgarter Str. 45, 70563 Stuttgart",
    rasen: "Kunstrasen", staerke: 2,
    name: "Michael Braun", tel: "+49 151 23456789", verein: "TSV Vaihingen",
    status: "offen", lat: 48.74, lng: 9.10,
  },
  {
    id: 2, type: "anfrage", jugend: "D-Jugend (U12)",
    datum: "2026-05-17", uhrzeit: "11:00",
    platz: "", adresse: "",
    rasen: "Naturrasen", staerke: 3,
    name: "Sarah Köhler", tel: "+49 171 98765432", verein: "FC Leinfelden",
    status: "offen", lat: 48.69, lng: 9.15,
  },
  {
    id: 3, type: "angebot", jugend: "C-Jugend (U14)",
    datum: "2026-05-24", uhrzeit: "14:00",
    platz: "Sportgelände West", adresse: "Hauptstr. 12, 70499 Stuttgart",
    rasen: "Naturrasen", staerke: 4,
    name: "Thomas Maier", tel: "+49 160 11223344", verein: "SV Weilimdorf",
    status: "offen", lat: 48.82, lng: 9.12,
  },
  {
    id: 4, type: "anfrage", jugend: "E-Jugend (U10)",
    datum: "2026-05-31", uhrzeit: "10:30",
    platz: "", adresse: "",
    rasen: "Kunstrasen", staerke: 2,
    name: "Anna Schmidt", tel: "+49 176 55667788", verein: "Spvgg Degerloch",
    status: "offen", lat: 48.73, lng: 9.19,
  },
  {
    id: 5, type: "angebot", jugend: "D-Jugend (U12)",
    datum: "2026-06-07", uhrzeit: "09:00",
    platz: "Waldstadion Böblingen", adresse: "Parkstr. 8, 71032 Böblingen",
    rasen: "Naturrasen", staerke: 3,
    name: "Klaus Fischer", tel: "+49 172 44556677", verein: "TSG Böblingen",
    status: "offen", lat: 48.68, lng: 9.01,
  },
];
...
function GameCard({ game, onClick }) {
  const dist = haversine(STUTTGART_LAT, STUTTGART_LNG, game.lat, game.lng);
  return (
    <div
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <Badge color="gray">{dist} km</Badge>
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
          ...
          {game.status === "gebucht" && <Badge color="green">✓ Gebucht</Badge>}
          <Badge color="gray">{dist} km entfernt</Badge>
        </div>
        ...
        {[
          ...
          { icon: "👤", label: "Trainer", value: game.name },
          { icon: "📞", label: "Telefon", value: game.tel },
        ].map((row) => (
    ...
    >
      ...
      >
        ...
        <div style={{ background: "#f5f5f5", borderRadius: 8, padding: "10px 14px", marginBottom: 16, fontSize: 13, color: "#666" }}>
          Bitte gib deine Kontaktdaten ein, damit {game.name} dich erreichen kann.
        </div>
...
function ListeTab({ games, bookings, onSelectGame }) {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "3rem 1rem", color: "#aaa" }}>Keine Einträge gefunden</div>
      ) : (
...
function EintragenTab({ onSubmit }) {
  ...
  const [form, setForm] = useState({
    ...
    platz: "", adresse: "", name: "", tel: "", verein: "",
  });

  function set(key, val) { setForm((f) => ({ ...f, [key]: val })); }

  function handleSubmit() {
    if (!form.datum || !form.name || !form.verein) {
      alert("Bitte Datum, Name und Verein ausfüllen.");
      return;
    }
    onSubmit({ ...form, type, staerke });
    setForm({ datum: "", uhrzeit: "10:00", jugend: "E-Jugend (U10)", rasen: "Naturrasen", platz: "", adresse: "", name: "", tel: "", verein: "" });
    setType("angebot");
    setStaerke(3);
  }
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
          <div>
            ...
            <input type="text" style={inputStyle} placeholder="Vor- Nachname" value={form.name} onChange={(e) => set("name", e.target.value)} />
          </div>
          <div>
            <label style={labelStyle}>Telefon</label>
            <input type="tel" style={inputStyle} placeholder="+49 ..." value={form.tel} onChange={(e) => set("tel", e.target.value)} />
          </div>
        </div>
        <div>
  ...
  function search() {
    const res = games
      .filter((g) => {
        ...
        if (haversine(STUTTGART_LAT, STUTTGART_LNG, g.lat, g.lng) > km) return false;
        return true;
      })
      .sort((a, b) =>
        haversine(STUTTGART_LAT, STUTTGART_LNG, a.lat, a.lng) -
        haversine(STUTTGART_LAT, STUTTGART_LNG, b.lat, b.lng)
      );
    setResults(res);
  }
...
export default function App() {
  ...
  // ── Login-Logik ──────────────────────────────────────────────────────────────
  useEffect(() => {
    // Beim Laden prüfen ob bereits eine Session existiert
    supabase.auth.getSession().then(({ data: { session } }) => {
      setSession(session);
    });

    // Auf Login / Logout automatisch reagieren
  ...
  function handleAddGame(formData) {
    const newGame = {
      ...
      id: Date.now(),
      status: "offen",
      lat: STUTTGART_LAT + (Math.random() - 0.5) * 0.2,
      lng: STUTTGART_LNG + (Math.random() - 0.5) * 0.2,
    };
